...
function HoverNeighborhood() {
  ...
  useEffect(() => {
    sigma.setSetting("nodeReducer", (node, data) => {
      ...
      const res: typeof data = { ...data, color: baseColor };
      ...
      // hovered node and neighbors keep cluster color and show label
      if (node === hoveredNode || neighbors?.has(node)) {
        res.forceLabel = true;
        return res;
      }

      // unrelated nodes greyed out and labels hidden
      res.color = "#eee";
      res.forceLabel = false;
      return res;
    });
...
export default function BoardgameGraph() {
  const boardgame_graph_data = useLoaderData() as Network;
  ...
  return (
    <div className="h-screen">
      ...
      >
        <LoadGraph data={boardgame_graph_data} />
        <GraphEvents />
        <NodeHoverCursor />
        <HoverNeighborhood />
      </SigmaContainer>
